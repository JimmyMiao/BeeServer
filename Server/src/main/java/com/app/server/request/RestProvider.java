...
public class RestProvider {
    ...
    /** 该方法必须是Map作为参数，将List的数据转换为sql语句一次性插入*/
    public String insertAllBusiness(Map map) {
        List<BusinessModel> list = (List<BusinessModel>) map.get("list");
        StringBuilder sb = new StringBuilder();
        sb.append("REPLACE INTO business ");
        sb.append("(id, name, image, startPrice, averageTime, monthSell) ");
        sb.append("VALUES ");
        MessageFormat mf = new MessageFormat("(#'{'list[{0}].id}, #'{'list[{0}].name}, #'{'list[{0}].image}," +
                " #'{'list[{0}].startPrice}, #'{'list[{0}].averageTime}, #'{'list[{0}].monthSell})");
        for (int i = 0; i < list.size(); i++) {
            sb.append(mf.format(new Object[]{i}));
            if (i < list.size() - 1) {
                sb.append(",");
